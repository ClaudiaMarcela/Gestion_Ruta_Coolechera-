...
/**
	...
	public class InfoPedidoFragment extends Fragment {
		 ...
		 */
		public static InfoPedidoFragment newInstance(int sectionNumber) {
			InfoPedidoFragment fragment = new InfoPedidoFragment();
		...
		public InfoPedidoFragment() {
		}

		@Override
		public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
			View rootView = inflater.inflate(R.layout.fragment_info_pedido,container, false);
			listViewProductosPedidos = (ListView) rootView.findViewById(R.id.listaProductosPedidos);
			
			icon = R.drawable.ic_av_shuffle_red;
			
			//cargar referencia y agregar captura de evento de busqueda.
			search=(SearchView) rootView.findViewById(R.id.searchViewBuscarProductoPedido);
		...
		public void cargarListaProductos(String text) {
			...
			listaReferencias = DataBaseBO.buscarReferenciasPedidas(text, this.getActivity().getBaseContext());
		 ...
		 * @param cantidad 
		 */
		protected void actualizarProductoEnDetalleVirtual(Referencias referencia, String cantidad) {
			...
			if(!numeroDoc.equals("ND")){
				...
				boolean insertado = DataBaseBO.actualizarProductoEnDetalleVirtual(referencia, cantidad, numeroDoc, getActivity().getBaseContext());
				if(insertado){
			...
			aceptarDialog.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(cantidad.length() > 0 && isNumber(cantidad)){
						...
						actualizarProductoEnDetalleVirtual(referencia, cantidad);
		...
		protected void mostrarAlertDialogEliminarProducto(final int position) {
			...
			setPositiveButton("Eliminar", new Dialog.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					eliminarReferenciaDeDetalleVirtual(referencia, getActivity().getBaseContext());
					dialog.cancel();
				}
		 ...
		 * @return
		 */
		protected void eliminarReferenciaDeDetalleVirtual(Referencias referencia, Context context) {			
			...
			if(!numeroDoc.equals("ND")){
				...
				boolean eliminado = DataBaseBO.eliminarProductoDeDetalleVirtual(referencia, numeroDoc, context);
				if(eliminado){
